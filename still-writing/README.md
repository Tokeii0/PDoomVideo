...
整支 MV 都是 Claude 在 Claude Code 里完成的，做法和根目录的 P(doom) MV 一样：

1. 先把歌词对上时间（见下一节），测出歌曲是 100 BPM。
2. 写好引擎、角色、场景、[分镜](STORYBOARD.md) 和给画师看的 [动画说明](ANIMATION_GUIDE.md)。
3. 十个章节交给十个并行的子代理分别画，每个只改自己的章节文件，用缩略图拼版反复检查、修改；它们报告的共享代码问题（图层内存泄漏、眉毛看不清、草稿线穿过脸等）再统一修掉。
4. 最后逐章检查、检查章节之间的转场，渲染全片。
